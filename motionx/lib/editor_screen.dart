...
class _MotionXStudioState extends State<MotionXStudio> {
  ...
  void toggleBeatMarker() {
    double cur = timeNotifier.value;
    int idx = beatMarkers.indexWhere((m) => (m - cur).abs() < 120);
    setState(() {
      if (idx >= 0) { beatMarkers.removeAt(idx); }
      else { beatMarkers.add(cur); beatMarkers.sort(); }
    });
  ...
  void updTotal() {
    double m = 60000.0;
    for (var l in layers) {
      if ((l.startMs + l.durMs + 10000.0) > m) m = l.startMs + l.durMs + 10000.0;
    }
    setState(() => totalMs = m);
  }
  void togglePlay() {
    ...
    timer?.cancel();
    if (play) {
      double maxT = layers.isEmpty ? 60000.0 : layers.map((l) => l.startMs + l.durMs).reduce(max);
      ...
      timer = Timer.periodic(const Duration(milliseconds: 33), (_) {
        ...
        double endT = layers.isEmpty ? 60000.0 : layers.map((l) => l.startMs + l.durMs).reduce(max);
        if (n >= endT) {
  ...
  void prevFrame() {
    ...
    double target = -1;
    for (var m in beatMarkers) {
      if (m < t - 20 && m > target) target = m;
    }
    if (target < 0) target = t - (1000 / widget.fps);
    seek(target);
  }

  void nextFrame() {
    double t = timeNotifier.value;
    double target = totalMs + 1000;
    for (var m in beatMarkers) {
      if (m > t + 20 && m < target) target = m;
    }
    if (target > totalMs) target = t + (1000 / widget.fps);
    seek(target);
  }
            ...
            Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                ...
                _txtBtn('Panjangkan', () {
                  ...
                  if (t > active!.startMs) {
                    saveState();
                    setState(() { active!.durMs = t - active!.startMs; });
                    updTotal();
                  }
                }),
                _txtBtn('Duplikat', () {
                ...
                Expanded(
                  child: ClipRect(
                    child: LayoutBuilder(builder: (_, box) {
                      ...
                      final double fullTrackW = (totalMs * pxPerMs) + w + 10000.0;
                      return Stack(fit: StackFit.expand, children: [
                        ...
                        ValueListenableBuilder<double>(
                          ...
                          builder: (_, t, __) => Stack(children: [
                            for (double m in beatMarkers)
                              Positioned(
                                left: cx + ((m - t) * pxPerMs) - 0.9, top: 0, bottom: 0,
                                child: IgnorePointer(child: Container(width: 1.8, color: const Color(0xFFFF2A6D))),
                              ),
                          ]),
                        ),
                        ...
                        Positioned(
                          left: cx - 36, top: 1,
                          child: GestureDetector(
                            onTap: toggleBeatMarker,
                            child: ValueListenableBuilder<double>(
                              valueListenable: timeNotifier,
                              builder: (_, t, __) {
                                bool isAtBeat = beatMarkers.any((m) => (m - t).abs() < 120);
                                return Container(
                                  width: 72, alignment: Alignment.center, padding: const EdgeInsets.symmetric(vertical: 1.5),
                                  decoration: BoxDecoration(color: const Color(0xFF1E2130), borderRadius: BorderRadius.circular(3), border: Border.all(color: isAtBeat ? const Color(0xFFFF2A6D) : const Color(0xFF00E676), width: 1.2)),
                                  child: Text(_fmt(t), style: TextStyle(color: isAtBeat ? const Color(0xFFFF2A6D) : const Color(0xFF00E676), fontSize: 9.5, fontFamily: 'monospace', fontWeight: FontWeight.bold)),
                                );
                              },
                            ),
                          ),
  ...
  Widget _buildBottomNavbar() {
    final List<Map<String, dynamic>> items = [
      {'icon': Icons.content_cut, 'label': 'Edit', 'id': 0},
      {'icon': Icons.music_note, 'label': 'Audio', 'id': 1},
      {'icon': Icons.title, 'label': 'Teks', 'id': 2},
      {'icon': Icons.sentiment_satisfied_alt_outlined, 'label': 'Stiker', 'id': 3},
      {'icon': Icons.auto_awesome_outlined, 'label': 'Efek', 'id': 4},
      {'icon': Icons.layers_outlined, 'label': 'Overlay', 'id': 5},
      {'icon': Icons.blur_on, 'label': 'Filter', 'id': 6},
      {'icon': Icons.tune, 'label': 'Sesuaikan', 'id': 7},
    ];

    return Container(
      height: 60,
      decoration: const BoxDecoration(color: Color(0xFF0B0D14), border: Border(top: BorderSide(color: Color(0xFF1D2132), width: 1))),
      child: SingleChildScrollView(
        ...
        child: Row(
          children: items.map((item) {
            ...
            final bool isSel = (_activeBottomNav == id);
            const Color green = Color(0xFF00E676);
            final Color itemColor = isSel ? green : Colors.white70;

            return InkWell(
              onTap: () {
                setState(() => _activeBottomNav = id);
                if (id == 1) pickMedia(false);
                if (id == 2) {
                  saveState();
                  setState(() {
                    layers.insert(0, LayerItem(id: '${DateTime.now().millisecondsSinceEpoch}', name: 'Teks', type: LayerType.text, color: const Color(0xFF7C4DFF), dur: 6000, start: timeNotifier.value));
                    sel = 0;
                  });
                  updTotal();
                }
                if (id == 3) {
                  saveState();
                  setState(() {
                    layers.insert(0, LayerItem(id: '${DateTime.now().millisecondsSinceEpoch}', name: 'Stiker', text: '🔥', type: LayerType.text, color: const Color(0xFFFFB300), dur: 5000, start: timeNotifier.value));
                    sel = 0;
                  });
                  updTotal();
                }
                if (id == 5) _showAddModal();
                if (id == 7) _openTr();
              },
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 6),
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Icon(item['icon'] as IconData, size: 22, color: itemColor),
                  const SizedBox(height: 3),
                  Text(item['label'] as String, style: TextStyle(fontSize: 10, fontWeight: isSel ? FontWeight.bold : FontWeight.w500, color: itemColor)),
                  const SizedBox(height: 3),
                  Container(width: 14, height: 2.5, decoration: BoxDecoration(color: isSel ? green : Colors.transparent, borderRadius: BorderRadius.circular(2))),
                ]),
              ),
            );
          }).toList(),
                ...
                GestureDetector(
                  ...
                  onHorizontalDragUpdate: (d) {
                    setState(() {
                      ...
                      if (l.durMs - del > 300 && l.startMs + del >= 0) { l.startMs += del; l.durMs -= del; }
                    });
                    updTotal();
                  },
                  child: Container(width: 22, color: Colors.white24, child: const Icon(Icons.chevron_left, size: 16, color: Colors.white)),
                ),
              Expanded(
                child: GestureDetector(
                  behavior: HitTestBehavior.opaque,
                  ...
                  child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: Text(l.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.bold))),
                ),
              ),
              if (isSel)
                GestureDetector(
                  ...
                  child: Container(width: 22, color: Colors.white24, child: const Icon(Icons.chevron_right, size: 16, color: Colors.white)),
                ),
            ]),
          ),
        ),
  ...
  Widget _renderCanvas(LayerItem l) {
    if (l.type == LayerType.video && l.vc != null && l.vc!.value.isInitialized) {
      final vd = l.vc!.value.duration.inMilliseconds.toDouble();
      ...
      builder: (_) => Padding(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          ...
          _btn(Icons.title, 'Teks', const Color(0xFF7C4DFF), () {
            ...
            saveState();
            setState(() { layers.insert(0, LayerItem(id: '${DateTime.now().millisecondsSinceEpoch}', name: 'Teks', type: LayerType.text, color: const Color(0xFF7C4DFF), dur: 6000, start: timeNotifier.value)); sel = 0; });
            updTotal();
          }),
        ]),
      ),
